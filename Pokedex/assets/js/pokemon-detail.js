let detail= (url)=>fetch(url).then((res)=>res.json()).then((data)=>{
  ...
  let pokemon_detail=`
    <h1>Pokedex</h1>
    <div id="card" class="card ${data.types[0].type.name} mx-auto" style="width: 25rem;">
      ...
      <div class="card-body">
        ...
        <div class="p-2 border ${data.types[0].type.name}">
        <h5>Stats</h5>
        ${stats}
        </div>
        <div class="p-2 border ${data.types[0].type.name}">
        ...
        <a id="backButton" class="btn btn-primary m-2">Voltar</a>
      </div>
    </div>
  `
